...
def build_args():
    ...
    parser.add_argument('--ignoreMEs', action='store_true', help='Ignore Mobile Elements (from 1KGP SV calls)')
...
def makefigure(df, sort_by, args):
    ...
    # plot each sample
    if args.matchcolors:
        colors = {'DEL': '#CC6677', 'ME DEL': '#CC6677', 'ME Del': '#CC6677', 'DUP': '#332288', 'INV': '#DDCC77', 'CNV': '#332288', 'INS': '#44AA99', 'ME INS': '#44AA99', 'ME Ins': '#44AA99', 'DEL (Jasmine)': '#CC6677', 'INS (Jasmine)': '#44AA99'}
        markers = {'DEL': '.', 'ME DEL': '.', 'ME Del': '.', 'ME Ins': '.', 'DUP': '.', 'INV': '.', 'CNV': '.', 'INS': '.', 'ME INS': '.', 'DEL (Jasmine)': 'D', 'INS (Jasmine)': 'D'}
    else:
        ...
        markers = {'DEL': 'o', 'ME DEL': 'o', 'DUP': 'o', 'INV': 'o', 'CNV': 'o', 'INS': 'o', 'ME INS': 'o', 'DEL (Jasmine)': 'X', 'INS (Jasmine)': 'X'}
    ...
    for (name, data) in df.iteritems():
        ...
        if(name == sort_by):
            continue
        ax.scatter(df.index, data, label=name, c=colors[name], marker=markers[name], s=1)
    #name = 'DEL'
    #ax.scatter(df.index, df[name], label=name, c=colors[name], marker=markers[name], s=1)
